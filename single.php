...
<body class="single">
   ...
   <div id="content-wrap">
   	...
   	<div class="row">
        ...
        INNER JOIN user ON user.uid = article.articleUser WHERE article.aid=$_GET[article] ORDER BY articleTime DESC", PDO::FETCH_ASSOC);
        if ($query->rowCount()) {
          foreach ($query as $row) { ?>
              ...
              <?php  }?>
                    ...
                    ?>"><strong>Previous Article</strong></a></li>
  				         <li class="next"><a rel="next" href="single.php?article=<?php echo $row["aid"]+1;?>"><strong>Next Article</strong></a></li>
  			        </ul>
        ...
        <?php
            ...
            <div id="comments">
               <!-- commentlist -->
                    ...
                    <?php
                    $query = $db -> query("SELECT * FROM comment WHERE comment.commentArticle=$_GET[article] AND commentPermission='1' ORDER BY commentTime DESC", PDO::FETCH_ASSOC);
                    if ($query->rowCount()) {
                      foreach ($query as $row) { ?>
